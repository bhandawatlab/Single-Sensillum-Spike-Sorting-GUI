...
hs = addcomponents;
    function hs = addcomponents
        ...
        set(gcf,'position',[10 150 750 600])
...
%         end
    ...
    function runOpt(hObject,event)
        ...
        if ~isempty(currStim)
            ...
            Optogenetics_ephys(currStim,fileNameStim{1},hs.pm.Value,10000);
            
            while isempty(DataExp)
            end
        else
            display('Stopped') % Added by Sam on 1/12/2021
        end
